...
async def migrate_data():
    ...
    async with aiosqlite.connect(database.DB_FILE) as db:
        ...
        if not os.path.exists("users.json"):
            print("users.json が見つかりません。スキップします。")
        else:
            try:
        ...
        if not os.path.exists("blackjack_data.json"):
            print("blackjack_data.json が見つかりません。スキップします。")
        else:
            try:
        ...
        if not os.path.exists("notification_settings.json"):
            print("notification_settings.json が見つかりません。スキップします。")
        else:
            try:
        ...
        if not os.path.exists("reminders.json"):
            print("reminders.json が見つかりません。スキップします。")
        else:
            try:
                async with aiofiles.open("reminders.json", "r", encoding="utf-8") as f:
                    reminders_raw = await f.read()
